...
const DESK_FRONT_RANGE_REDUCTION_RATIO = 0.2;
...
function reduceDeskFrontBounds(bounds, ratio = DESK_FRONT_RANGE_REDUCTION_RATIO) {
  if (!bounds) return bounds;
  const depth = bounds.maxZ - bounds.minZ;
  if (!Number.isFinite(depth) || depth <= 0) return bounds;
  return {
    ...bounds,
    maxZ: bounds.maxZ - (depth * ratio),
  };
}
...
function buildDeskAndDecor() {
  ...
  if (analyzedSurface) {
    ...
    STATE.room.deskSurfaceBounds = reduceDeskFrontBounds({
...
function getDeskSurfaceBounds() {
  return STATE.room.deskSurfaceBounds || reduceDeskFrontBounds({
    minX: -1.55,
    maxX: 1.55,
    minZ: -5.8,
    maxZ: -4.55,
