...
const DESK_FRONT_RANGE_REDUCTION_RATIO = 0.92;
...
const DESK_MOTION_BACK_REDUCTION_RATIO = 0.22;
...
function reduceDeskDepthBounds(
  bounds,
  frontRatio = DESK_FRONT_RANGE_REDUCTION_RATIO,
  extraRemainingRatio = DESK_FRONT_EXTRA_REMAINING_REDUCTION_RATIO,
  backRatio = DESK_BACK_RANGE_REDUCTION_RATIO,
) {
  if (!bounds) return bounds;
  const depth = bounds.maxZ - bounds.minZ;
  if (!Number.isFinite(depth) || depth <= 0) return bounds;

  const clampedFrontRatio = clamp(frontRatio, 0, 0.98);
  const remainingDepth = depth * (1 - clampedFrontRatio);
  const extraFrontReduction = remainingDepth * clamp(extraRemainingRatio, 0, 0.95);
  let nextMaxZ = bounds.maxZ - (depth * clampedFrontRatio) - extraFrontReduction;

  const clampedBackRatio = clamp(backRatio, 0, 0.95);
  let nextMinZ = bounds.minZ + (depth * clampedBackRatio);

  const minRemainingDepth = Math.max(0.03, DESK_MIN_REMAINING_DEPTH);
  if (nextMinZ > nextMaxZ - minRemainingDepth) {
    nextMinZ = nextMaxZ - minRemainingDepth;
  }

  nextMinZ = clamp(nextMinZ, bounds.minZ, bounds.maxZ - minRemainingDepth);
  nextMaxZ = Math.max(nextMinZ + minRemainingDepth, nextMaxZ);
...
function buildDeskAndDecor() {
  ...
  if (analyzedSurface) {
    ...
    STATE.room.deskSurfaceBounds = reduceDeskDepthBounds({
...
function getDeskSurfaceBounds() {
  return STATE.room.deskSurfaceBounds || reduceDeskDepthBounds({
    minX: -1.55,
    maxX: 1.55,
    minZ: -5.8,
    maxZ: -4.55,
...
function updatePhysics(delta) {
  ...
  STATE.visuals.forEach((visual) => {
    ...
    const speedBand = p.friction <= 0.8 ? 13.5 : p.friction <= 0.9 ? 5.6 : 0.78;
    const tiltScale = p.onDesk ? 0.56 : 1.0;
    const accelMultiplier = (p.onDesk ? 16.0 : 32.0) * speedBand;
    ...
    /* Clamp max velocity */
    const maxVelocity = (p.onDesk ? PHYSICS_MAX_VELOCITY * 0.66 : PHYSICS_MAX_VELOCITY) * speedBand;
