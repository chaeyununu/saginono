...
const COARSE_INITIAL_SCENE_MEMO_LIMIT = 12;
const COARSE_PROGRESSIVE_SCENE_BATCH = 8;
const COARSE_PROGRESSIVE_SCENE_DELAY_MS = 480;
const RENDERER_PIXEL_RATIO_CAP = IS_COARSE_POINTER_DEVICE ? 1.12 : 1.8;
const SHADOW_MAP_SIZE = IS_COARSE_POINTER_DEVICE ? 1024 : 2048;
const DISABLE_HOVER_PROXY_ON_COARSE = IS_COARSE_POINTER_DEVICE;
...
const STATE = {
  ...
  appReady: false,
  loadingOverlay: null,
  sceneMemoRenderLimit: IS_COARSE_POINTER_DEVICE ? COARSE_INITIAL_SCENE_MEMO_LIMIT : Number.POSITIVE_INFINITY,
  progressiveSceneHydrationScheduled: false,
...
const DUMMY_MEMO_TARGET_COUNT = 40;
...
function ensureSeedDummyMemos(targetCount = DUMMY_MEMO_TARGET_COUNT) {
  ...
  if (STATE.memos.length > 0 || activeDummyCount >= targetCount) return false;
...
async function init() {
  ...
  STATE.appReady = true;
  hideLoadingOverlay();
  scheduleProgressiveSceneHydration();
}
...
function setupScene() {
  ...
  STATE.renderer = new THREE.WebGLRenderer({ antialias: !IS_COARSE_POINTER_DEVICE, alpha: false });
  STATE.renderer.setPixelRatio(Math.min(window.devicePixelRatio || 1, RENDERER_PIXEL_RATIO_CAP));
  STATE.renderer.setSize(width, height);
  STATE.renderer.outputColorSpace = THREE.SRGBColorSpace;
  STATE.renderer.shadowMap.enabled = true;
  STATE.renderer.shadowMap.type = IS_COARSE_POINTER_DEVICE ? THREE.PCFShadowMap : THREE.PCFSoftShadowMap;
...
async async function loadAssets() {
  const loader = new GLTFLoader();
  const initialSourceMemos = getRenderableSceneMemos(STATE.memos, IS_COARSE_POINTER_DEVICE ? COARSE_INITIAL_SCENE_MEMO_LIMIT : Number.POSITIVE_INFINITY);
  const initialKeys = getRequiredAssetKeysForMemos(initialSourceMemos);
...
function createMemoHoverProxy(object) {
  if (DISABLE_HOVER_PROXY_ON_COARSE) return null;
...
function rebuildVisuals(animateMemoId = null) {
  ...
  const activeMemos = getRenderableSceneMemos(STATE.memos);
  const now = Date.now();
